...
const postbackKey = process.env.CPAGRIP_POSTBACK_KEY;
...
export async function GET(req: NextRequest) {
  if (!postbackKey) {
    return NextResponse.json(
      { error: "CPA Grip postback key not configured." },
      { status: 500 }
    );
  }
  ...
  if (!userId) {
    return NextResponse.json(
      { error: "Missing user identifier in CPA Grip postback." },
      { status: 400 }
    );
  }
  ...
  if (!payout || payout <= 0) {
    return NextResponse.json(
      { error: "Invalid CPA Grip payout amount." },
      { status: 400 }
    );
  }
  ...
  const externalId =
    ...
    `cpagrip-${offerId}-${timestamp || Date.now()}`;
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!account) {
        return {
          status: 404,
          body: { error: "User not found for CPA Grip postback." },
        };
      }
    ...
    return NextResponse.json(
      { error: "Failed to process CPA Grip postback." },
      { status: 500 }
    );
  }
}
